        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            // define a path made up of (x,y) points that the car will visit sequentially every .02 seconds
          	...
          	// if the previous path size is almost empty, use the car as the starting reference
          	if(prev_size < 2)
          	{
          	  // use two point that makes the path tangent to the car
          	...
          	{
          	  // redefine the reference state as prevous path end point
            ...
            // define the actual (x, y) points that we will use for the planner
          	...
          	{
          	  double N = target_dist / (0.02 * ref_vel / 2.24); // 2.24 is for mph to m/s
